...
# Project Imports
from doc_viewer.domain.events import document
...
from doc_viewer.domain.events.category.category_events import (
    ...
    DocumentToggleFavouritedCategoryEvent,
    DocumentRemovedCategoryEvent
)
...
class Category(QTreeWidgetItem):
    ...
    def subscribe_events(self):
        ...
        event_bus.subscribe(DocumentRemovedCategoryEvent, self.handle_events)
        event_bus.subscribe(DocumentToggleFavouritedCategoryEvent, self.handle_events)
    ...
    def handle_events(self, event):
        ...
        if isinstance(event, DocumentAddedCategoryEvent):
            if event.get_category_name() == self._name:
                self.add_document(event.get_document_file_path())
        elif isinstance(event, DocumentToggleFavouritedCategoryEvent):
            if self.get_name() == "Favourites":
                document_path = event.get_document_file_path()
                ui_document = document_catalog.get_document(document_path)
                is_favourited = event.is_favourited()
                if is_favourited:
                    self.add_document(document_path)
                else:
                    self.remove_document(document_path)
                ui_document.set_favourited(is_favourited)
        elif isinstance(event, DocumentRemovedCategoryEvent):
            if event.get_category_name() == self._name:
                self.remove_document(event.get_document_file_path())
    ...
    def remove_document(self, document_path: str) -> bool:
        ...
        Args:
            document_path (str): The path of the document to remove.
        ...
        thumbnail_panel = self.get_thumbnail_panel()
        documents = self.get_documents()
        if document_path in documents.keys():
            del self._documents[document_path]
            thumbnail_panel.remove_thumbnail_viewer(document_path)
            return True
        return False
